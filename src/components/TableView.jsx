...
const TableView = ({ section = 'individu', setSection, data, page, total, limit, setLimit, setPage, fetchData, setSelectedIndividu, activeFilters = [] }) => {
  ...
  const columns = useMemo(() => {
    const commonFlagCell = (info) => {
      ...
      return (
        <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${isSangatMiskin ? 'bg-rose-50 border-rose-200 text-rose-700 shadow-[0_2px_10px_-3px_rgba(225,29,72,0.3)]' : isMiskin ? 'bg-amber-50 border-amber-200 text-amber-700 shadow-[0_2px_10px_-3px_rgba(217,119,6,0.3)]' : 'bg-slate-50 border-slate-200 text-slate-700'}`}>
      ...
      <div className="flex-1 bg-white/70 backdrop-blur-xl rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 overflow-hidden flex flex-col relative z-0">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between w-full">
            ...
            <div className="flex items-center gap-3 self-end sm:self-auto">
              <div className="relative">
                <select value={section} onChange={(e) => setSection(e.target.value)} className="appearance-none border border-blue-200 rounded-xl pl-4 pr-10 py-2.5 text-sm bg-blue-50/50 text-blue-700 font-bold hover:bg-blue-100/50 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer shadow-sm">
              ...
              <div className="flex gap-2.5">
                <button onClick={() => handleExport('csv')} className="flex gap-2 items-center cursor-pointer bg-gradient-to-r from-slate-700 to-slate-800 hover:from-slate-800 hover:to-slate-900 text-white rounded-xl px-4 py-2.5 text-sm font-semibold shadow-md shadow-slate-400/20 hover:shadow-lg transform hover:-translate-y-0.5 transition-all">
                  <FileText size={16} /> <span className="hidden sm:inline">CSV</span>
                </button>
                <button onClick={() => handleExport('xlsx')} className="flex gap-2 items-center cursor-pointer bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white rounded-xl px-4 py-2.5 text-sm font-semibold shadow-md shadow-emerald-400/20 hover:shadow-lg transform hover:-translate-y-0.5 transition-all">
                  <FileSpreadsheet size={16} /> <span className="hidden sm:inline">Excel</span>
                </button>
                <button onClick={() => handleExport('pdf')} className="flex gap-2 items-center cursor-pointer bg-gradient-to-r from-rose-500 to-red-600 hover:from-rose-600 hover:to-red-700 text-white rounded-xl px-4 py-2.5 text-sm font-semibold shadow-md shadow-rose-400/20 hover:shadow-lg transform hover:-translate-y-0.5 transition-all">
          ...
          {selectedIds.length > 0 && (
            <div className="p-3 px-4 bg-gradient-to-r from-indigo-50 to-blue-50 border border-indigo-100/50 rounded-xl shadow-inner flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between animate-in fade-in slide-in-from-top-2">
              ...
              <div className="flex flex-wrap gap-2.5 justify-center sm:justify-end">
                ...
                <button onClick={handleDownloadSelectedCards} disabled={bulkDownloading} className="px-4 py-2 bg-indigo-600 text-white border border-transparent text-sm rounded-lg hover:bg-indigo-700 disabled:opacity-50 font-bold transition-colors shadow-sm shadow-indigo-200">
        ...
        <div className="p-4 border-t border-slate-100 flex flex-col sm:flex-row gap-4 justify-between items-center bg-white/60 backdrop-blur-md">
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-center justify-center flex-1">
            <select value={limit} onChange={(e) => handleLimitChange(Number(e.target.value))} className="border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold bg-white text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm cursor-pointer">
