...
const Sidebar = ({ activeFilters, columnOptions, tableSection, addFilter, removeFilter, updateFilter, fetchOptions, fetchData, loading, isOpen }) => {
  ...
  // PERBAIKAN: Langsung ambil dari Object.keys jika tab keluarga
  const currentAvailableColumns = tableSection === 'keluarga' ? Object.keys(keluargaColumnsAliases) : masterFilterColumns;
  ...
  return (
    ...
    >
                            ...
                            <div className="space-y-4 bg-white p-3 border border-slate-100 rounded-xl shadow-sm">
                              ...
                              <div className="w-full flex items-center text-center gap-3">
                                <input
                                  ...
                                  max={rangeMax}
                                  value={safeMin}
                                  onChange={(e) => updateFilter(idx, 'value', [Number(e.target.value), safeMax])}
                                />
                                <input
                                  className="w-16 bg-slate-50 rounded-lg border-slate-200 border p-1 text-xs text-center font-bold text-slate-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                                  type="number"
                                  value={safeMin}
                                  onChange={(e) => updateFilter(idx, 'value', [Number(e.target.value), safeMax])}
                                />
                              ...
                              <div className="w-full flex items-center text-center gap-3">
                                <input
                                  ...
                                  max={rangeMax}
                                  value={safeMax}
                                  onChange={(e) => updateFilter(idx, 'value', [safeMin, Number(e.target.value)])}
                                />
                                <input
                                  className="w-16 bg-slate-50 rounded-lg border-slate-200 border p-1 text-xs text-center font-bold text-slate-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                                  type="number"
                              ...
                              {optionValues.map((option) => {
                                ...
                                return (
                                  <label key={value} className="flex items-start gap-3 p-2 hover:bg-slate-50 rounded-lg cursor-pointer transition-colors group">
                                    ...
                                    <span className={`text-xs flex-1 leading-snug transition-colors ${isChecked ? 'font-bold text-blue-700' : 'font-medium text-slate-600 group-hover:text-slate-800'}`}>{value}</span>
                                  </label>
                                );
                              })}
                            </div>
      ...
      <div className="p-6 mt-auto border-t border-slate-100 bg-white/50 backdrop-blur-sm shadow-[0_-4px_24px_rgb(0,0,0,0.02)]">
        <button
          onClick={() => fetchData(1)}
          className="relative w-full overflow-hidden bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold py-3.5 rounded-xl hover:from-orange-600 hover:to-orange-700 shadow-lg shadow-orange-500/20 active:scale-[0.98] transition-all group flex items-center justify-center gap-2"
        >
          {loading ? (
            <div className="flex items-center gap-2">
